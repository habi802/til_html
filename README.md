...
- [설치](https://nodejs.org)
  ...
  - `추후 NVM(Node Version Manager)으로 버전 관리`
...
```
common.css, header.css, footer.css 등 영역 별로 생성
```

- `프로젝트명/public/js` 폴더 생성

```
각 종류의 라이브러리.js, header.js, footer.js 등 영역 별로 생성
```
...
- 시멘틱 태그: 태그의 단어가 내용을 설명하는 역할을 말함.
...
### 2.4. 위 구조 기본 작성 후 각 내용을 div 태그로 분할
...
### 1.2. VSCode에서 Git 관리하기

- 터미널 입력창을 Git Bash로 셋팅해서 관리
- Git Bash를 선택하는 이유는 Linux, Mac 운영체제의 터미널 사용

```
관리도구 버튼 > Setting > 입력창 > default:Windows > Default Profile: Windows에 Git Bash 선택
```

- 설정 확인

```
VSCode 주메뉴 > Terminal > Terminal 보기 선택 > Bash 확인
```

### 1.3. 기본 저장소(Repository) 이름 설정하기

- 옛날에는 브랜치명이 `master 라고 생성`이 되었으나
- 현대에는 브랜치명이 `main`으로 생성하여야 함
